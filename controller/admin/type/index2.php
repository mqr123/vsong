...
if(!empty($this->param[1])&&$this->param[1]=='post'){
	...
	if($result['type']=='success'){
		...
		echo "<script>alert('修改成功');location.href='../index2/".$upid."';</script>";
		exit;
	}
	else{echo "<script>alert('修改失败');location.href='../index2/".$upid."';</script>";
		exit;
	}
	...
	$er=$one->field('id,name')->where('id='.$id)->search();
