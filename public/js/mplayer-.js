...
function MidiFile(data, singleChannel) {
	...
	for (var i = 0; i < header.trackCount; i++) {
		...
		while (!trackStream.eof()) {
            ...
            if (singleChannel) {
                if (event.channel == singleChannel - 1){
					tracks[i].push(event);
            ...
            if (event.subtype == "setTempo") {
                header.tempo = 6e7 / event.microsecondsPerBeat;
            }
		}
	}
...
midi.loadMidiFile = function(onsuccess, onprogress, onerror) {
	try {
		var MidiData = MidiFile(midi.currentData, window.singleChannel || MIDI.singleChannel);
		midi.tempo = MidiData.header.tempo;
		midi.BPM = MidiData.header.tempo;
		midi.ticksPerBeat = MidiData.header.ticksPerBeat;
		midi.progress = {value:0,max:MidiData.header.total};
		midi.replayer = new Replayer(MidiData, midi.timeWarp, null, midi.BPM);
